...
if (intval($_GET['delete']) == 0) {
    ...
    $id=$_GET['delete'];
    $delete_qry = "DELETE FROM business_declaration 
                   WHERE business_declaration_id ='".$id."' LIMIT 1";
    $stmt = $dbh->prepare($delete_qry);
    if($stmt->execute()){
        $msg='Record deleted successfully.';
		echo "<meta http-equiv='refresh' content='0;url=../../staffUser.php?dir=businessAsset&page=business&success_msg=".$msg."'>";
    } else {
        $msg='Deletion failed. Please contact Administrator.';
		echo "<meta http-equiv='refresh' content='0;url=../../staffUser.php?dir=businessAsset&page=business&error_msg=".$msg."'>";
    }
}
