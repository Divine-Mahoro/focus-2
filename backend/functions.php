...
function getDeclarationPeriodId(){
	global $dbh;
	$currentDate = date("Y-m-d");	
	$qry = "SELECT * FROM declaration_periods WHERE start_date <= '".$currentDate."' and end_date >= '".$currentDate."'";
	$stmt = $dbh->prepare($qry);
	$stmt->execute();
	if($res = $stmt->fetch(PDO::FETCH_ASSOC)){
		return $res['declaration_period_id'];
	}
	return NULL;
}

function getAssetCity($imm_asset_id = NULL){
	//global $dbh;
	if($imm_asset_id != NULL){
		return (getFieldValueById("city", "immovable_asset_declarations", $imm_asset_id));
	}
	return NULL;
}

function getAssetCityLivestock($m_asset_id = NULL){
	//global $dbh;
	if($m_asset_id != NULL){
		return (getFieldValueById("city", "livestock_declarations", $m_asset_id));
	}
	return NULL;
}

function getAssetCityOtherMovable($m_asset_id = NULL){
	//global $dbh;
	if($m_asset_id != NULL){
		return (getFieldValueById("city", "other_movable_declarations", $m_asset_id));
	}
	return NULL;
}
